...
function NavSection({ title, items, user, pathname, onClose }: { title: string; items: NavItem[]; user: any; pathname: string; onClose: () => void }) {
  const visible = items.filter(({ producerOnly, adminOnly }) => {
    if (producerOnly && !user?.isProducer && !user?.isAdmin) return false;
  ...
  return (
    <div style={{ marginBottom: 6 }}>
      <p style={{ fontSize: 9.5, fontWeight: 700, color: 'rgba(255,255,255,0.2)', textTransform: 'uppercase', letterSpacing: '0.12em', padding: '0 10px', margin: '0 0 6px' }}>
        {title}
      </p>
      {visible.map(({ href, label, icon: Icon }) => {
        const active = pathname === href || (
          href !== '/dashboard' && href !== '/rede' &&
          pathname.startsWith(href + '/') &&
          !visible.some(o => o.href !== href && (pathname === o.href || pathname.startsWith(o.href + '/')))
        );
        return (
          <Link key={href} href={href} onClick={onClose}
            className={cn(!active && 'sb-link')}
            style={{
              display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 10, textDecoration: 'none',
              fontSize: 13.5, fontWeight: active ? 600 : 500,
              color: active ? '#c3e438' : 'rgba(255,255,255,0.5)',
              background: active ? 'rgba(195,228,56,0.1)' : 'transparent',
              transition: 'all 0.15s',
            }}>
            <div style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: active ? 'rgba(195,228,56,0.15)' : 'rgba(255,255,255,0.05)', flexShrink: 0 }}>
              <Icon size={14} />
            </div>
            <span style={{ flex: 1 }}>{label}</span>
            {active && <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#c3e438' }} />}
          </Link>
        );
      })}
    </div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.07)', padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
          {user && (
            <Link href="/profile" onClick={onClose} style={{ textDecoration: 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer' }} className="sb-link">
          ...
          <button onClick={logout} className="sb-logout"
            ...
            <div style={{ width: 28, height: 28, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.04)', flexShrink: 0 }}>
              <LogOut size={14} />
            </div>
            Sair da conta
          </button>
